...
export default function StopTimerModal({ open, onClose, onSave, data, clients }) {
  ...
  const handleSave = async (e) => {
    ...
    await onSave({
      ...
      date, durationMinutes: totalMin, billable, notes: notes.trim(),
    });
    setSaving(false);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="bg-white rounded-[14px] w-[440px] max-h-[90vh] overflow-y-auto p-8 animate-modal-in" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-[#5777AB] uppercase tracking-[0.06em] mb-1">Billable</label>
            <div className="flex items-center gap-3 pt-1">
              <label className="flex items-center gap-1.5 cursor-pointer">
                <input type="radio" name="billable-modal" checked={!billable} onChange={() => setBillable(false)} className="accent-[#8403C5] w-4 h-4" />
                <span className="text-sm text-[#242450]">No</span>
              </label>
              <label className="flex items-center gap-1.5 cursor-pointer">
                <input type="radio" name="billable-modal" checked={billable} onChange={() => setBillable(true)} className="accent-[#8403C5] w-4 h-4" />
          ...
          <div>
            <label className="block text-xs font-semibold text-[#5777AB] uppercase tracking-[0.06em] mb-1">Notes</label>
