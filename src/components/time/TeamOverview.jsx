...
export default function TeamOverview({ refresh }) {
  ...
  const projectBreakdown = useMemo(() => {
    const map = {};
    filtered.forEach(e => {
      const key = `${e.category}|||${e.projectTask}`;
      if (!map[key]) map[key] = { category: e.category, task: e.projectTask, minutes: 0 };
  ...
  // CSV export
  const exportCSV = () => {
    const header = 'Date,Team Member,Category,Project/Task,Duration (hours),Billable,Notes';
    const rows = filtered.map(e =>
      `"${e.date}","${e.teamMember}","${e.category}","${(e.projectTask || '').replace(/"/g, '""')}","${fmtDecimal(e.durationMinutes)}","${e.billable ? 'Yes' : 'No'}","${(e.notes || '').replace(/"/g, '""')}"`
    );
      ...
      <div>
        ...
        <div className="bg-white border border-[#EBEBF5] rounded-xl overflow-hidden">
          <table className="w-full text-sm">
            <thead>
              <tr>
                {['Category', 'Project / Task', 'Total hours'].map(h => (
                  <th key={h} className="px-4 py-3 text-left text-[11px] font-bold text-[#5777AB] uppercase tracking-[0.08em]">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {projectBreakdown.length === 0 ? (
                <tr><td colSpan={3} className="px-4 py-12 text-center text-sm text-[#5777AB]">No data for this period</td></tr>
              ) : (
