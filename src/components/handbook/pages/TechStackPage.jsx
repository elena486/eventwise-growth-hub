    ...
    <HandbookPageShell section={section} page={page} onUpdate={onUpdate} onDelete={onDelete}>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-[#242450]">
              {['Tool', "What it's for", 'Owner', 'Status', ''].map(h => (
                <th key={h} className="px-4 py-3 text-left text-[11px] font-semibold text-white uppercase tracking-[0.1em] whitespace-nowrap first:rounded-tl-lg last:rounded-tr-lg">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {tools.map((row, i) => (
              <tr key={row.id} className={`group border-b border-[#E5E7EB] last:border-0 hover:bg-[#F3F4F6] transition-colors ${i % 2 === 1 ? 'bg-[#F9FAFB]' : 'bg-white'}`}>
                {editId === row.id ? (
                  <>
                  ...
                  <>
                    <td className="px-4 py-3">
                     <div className="flex items-center gap-1.5">
                       <span className="font-semibold text-[#242450] text-xs">{row.tool}</span>
                       {row.url && (
                    ...
                    <td className="px-4 py-3 text-[#374151] text-xs max-w-[240px]">{row.what}</td>
                    <td className="px-4 py-3 text-[#374151] text-xs whitespace-nowrap">{row.owner}</td>
                    <td className="px-4 py-3">
