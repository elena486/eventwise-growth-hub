...
export function generateReportPDF(report, prevReport) {
  ...
  function sectionTitle(x, y, icon, title) {
    doc.setFillColor(...PURPLE);
    doc.rect(x, y, 2.5, 7, 'F');
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(11);
    doc.setTextColor(...NAVY);
    doc.text(`${icon}  ${title}`, x + 6, y + 5.5);
    return y + 12;
  }
  ...
  function statBlock(x, sy, label, value, prevVal) {
    ...
    if (m) {
      ...
      doc.setFont('helvetica', 'bold');
      doc.text(`${m.up ? '↑' : '↓'} ${m.pct}%`, x, sy + 14.5);
    } else {
      doc.setFontSize(7);
      doc.setTextColor(180, 180, 200);
      doc.text('No prior data', x, sy + 14.5);
    }
    return sy + 18;
  }
  ...
  let ly = sectionTitle(LX + 4, y + 3, '🌐', 'Website');
  // 4 stats in a row
  ...
  let nly = sectionTitle(LX + 4, NL_Y + 3, '✉️', 'Newsletter — Beehiiv');
  ...
  let ry = sectionTitle(RX + 4, y + 3, '📊', 'Chris LinkedIn — Personal');
  const liW = (COL_W - 8) / 4;
  ...
  if (li.topPostTitle) {
    ...
    doc.setTextColor(...NAVY);
    doc.text(doc.splitTextToSize(li.topPostTitle, COL_W - 30)[0], RX + 25, ry + 4);
    ry += 9;
  }
  ...
  let cy = sectionTitle(RX + 4, CP_Y + 3, '🏢', 'Eventwise Company Page');
  const cpW = (COL_W - 8) / 4;
