...
export default function AppShell() {
  ...
  return (
    <div className="flex flex-col h-screen font-dm overflow-hidden">
          ...
          <div className="relative flex items-center gap-2.5 shrink-0" ref={hqRef}>
            ...
            >
              ...
              {recentItems.length > 0 && (
                <span className="inline-flex items-center justify-center min-w-[18px] h-[18px] text-[10px] font-bold bg-[#7C3AED] text-white rounded-full px-1.5 leading-none">
                  {recentItems.length}
                </span>
              )}
            ...
            {hqOpen && (
              <div className="absolute left-0 top-full mt-1.5 w-[280px] bg-white dark:bg-[#1A1A2E] rounded-[10px] border border-[#F0F0F0] dark:border-[#2D2D44] z-50 overflow-hidden" style={{boxShadow:'0 4px 16px rgba(0,0,0,0.08)'}}>
                <div className="px-4 py-3 border-b border-[#F0F0F0] dark:border-[#2D2D44]">
                ...
                {recentItems.length === 0 ? (
                  ...
                  <div>
                    {recentItems.map((item, i) => {
                      ...
                      return (
                        <button
                          ...
                          className="w-full flex items-center gap-3 px-4 h-[52px] hover:bg-[#FAFAFA] dark:hover:bg-[#1F1F38] transition-colors text-left"
                        >
                    ...
                    <div className="border-t border-[#F0F0F0] dark:border-[#2D2D44] px-4 py-2.5">
                      <button
                        onClick={() => {
        ...
        <div className="flex items-center gap-2 shrink-0 ml-4">
          ...
          {user && (
            <div className="relative" ref={avatarRef}>
              ...
              {avatarOpen && (
                <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-[10px] border border-[#F0F0F0] overflow-hidden z-50" style={{boxShadow:'0 4px 16px rgba(0,0,0,0.08)'}}>
                  <div className="px-4 py-3 border-b border-[#F0F0F0]">
                    <p className="text-sm font-semibold text-[#0F0F1A]">{user.full_name || '—'}</p>
                    <p className="text-xs text-[#9CA3AF] mt-0.5 truncate">{user.email}</p>
                  </div>
                  <button
                    onClick={() => { setDark(d => !d); }}
                    className="w-full flex items-center justify-between px-4 py-2.5 text-sm text-[#374151] hover:bg-[#FAFAFA] transition-colors"
                  >
                    <span className="flex items-center gap-2">
                      {dark ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4" />}
                      {dark ? 'Light mode' : 'Dark mode'}
                    </span>
                    <span className={`w-8 h-4 rounded-full transition-colors ${dark ? 'bg-[#7C3AED]' : 'bg-gray-200'} relative inline-block`}>
                  ...
                  {(user?.email || '').toLowerCase().includes('elena') && (
                    <button
                      ...
                      className="w-full flex items-center gap-2 px-4 py-2.5 text-sm text-[#374151] hover:bg-[#FAFAFA] transition-colors"
                    >
                  ...
                  <button
                    ...
                    className="w-full flex items-center gap-2 px-4 py-2.5 text-sm text-[#DC2626] hover:bg-[#FEF2F2] transition-colors border-t border-[#F0F0F0]"
                  >
      ...
      {/* Content */}
      <div className="flex-1 overflow-hidden flex bg-[#FAFAFA] dark:bg-[#0F0F1A]">
      ...
      <button
        ...
        className="fixed bottom-5 right-5 w-9 h-9 flex items-center justify-center rounded-full bg-white border border-[#F0F0F0] text-[#9CA3AF] hover:text-[#6B7280] hover:border-[#E5E7EB] transition-all z-40"
