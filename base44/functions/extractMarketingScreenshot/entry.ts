...
const PROMPTS = {
  ...
  linkedinPost: `You are reading a single LinkedIn post's analytics screenshot (the per-post analytics view that shows metrics like impressions, reactions, comments, reposts, link clicks, profile visits, and reach). Extract every clearly visible number and return ONLY a JSON object with these exact keys (use null for any not visible): impressions, reactions, comments, reposts, link_clicks, profile_visits, reach. Do NOT guess, estimate, or default to zero — if a stat is blurry, cropped, partially cut off, or simply not shown in the screenshot, use null. Only return numbers you can read with confidence. After the JSON on a new line write NARRATIVE: followed by a one-sentence note listing which of the seven fields were not visible in the screenshot.`,
};
